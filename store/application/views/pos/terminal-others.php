    ...
    <div class="login-form-area mg-t-30 mg-b-30">
        <div class="col-lg-12">
            <div class="row checkout-header ng-scope">
                ...
                <div class="col-lg-2">
                    <button style="margin-top:-40px" onclick="return LoadPendingcart();" class="btn btn-sm btn-default">
                        <?php
                        $count =0;
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <button class="btn btn-default" type="button"  data-toggle="modal" data-target="#InformationproModalalert"> <i class="fa fa-pencil"></i> <span class="hidden-sm hidden-xs">Note</span></button>                    
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <form action="<?php echo base_url("dashboard/".$endpoint);  ?>" id="cart_form" method="POST">
                                    <div class="direct-chat-messages" id="cart-table-body" style="padding: 0px; height: 280px; overflow:auto;">
                            ...
                            <table class="table" id="cart-details">
                                ...
                                <tr class="active">
                                    <td></td>
                                    <td></td>
                                    <td class="text-right cart-discount-notice-area">VAT</td>
    ...
    $(document).ready(function(){
        ...
        $('#search_bar').typeahead({
            source: function(query, process) {
                ...
                $.ajax({
                    url: '<?php  echo base_url("dashboard/getProductBySearchService?barcode=") ?>'+textVal,
                    type: 'POST',
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row" id="cash_sales">
                    ...
                    <br/>
                    <div class="text-center"><button class="btn btn-lg btn-primary" id="cashpnow" onclick="$('#cashsales').modal('show'); return PayNow(true)">Print Receipt</button></div>
                </div>
            </div>
        </div>
    </div>
...
//to_pay = to_pay+calculateVat();
            ...
            if(data['method'] == "2" && dialog ==false && data['sales_type']!="PICKUP") {
                ...
                ajaxSentRequest($("#cart_form").attr('action'), data, function (result) {
                    ...
                    if (result.status == true) {
                        ...
                        window.open(result.print, 'width=400;height=400;', '400', '400');
    ...
    function getInputFromBarcode(code,qty){
        if(swal.isVisible()){
            ...
            $.get('<?php  echo base_url("dashboard/getServiceAssociatedWithBarcode?barcode=") ?>'+code,function(result){
                result = JSON.parse(result);

                if(result.status == false){;
                    error(result.message)
                }else if(result.quantity == 0 && result.track =="1"){
                    error("Item Out of Stock");
            ...
            if(data.status == true){
                ...
                for(var i=0; i < items.length;i++){
                    for(var u=0; u < items[i].item_qty; u++) {
                        getInputFromBarcode(items[i].id,1);
                    }
                }
                continue_id = cart_id;
