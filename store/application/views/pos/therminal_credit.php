...
<div class="row checkout-header ng-scope">
				...
				<div class="col-lg-4" style="margin-bottom:10px">
				  ...
				  ?>
				   <center>  <button style="margin-top:-40px" onclick="goTo( '<?php echo base_url('dashboard/credit'); ?>' )" class="btn btn-sm btn-default">
				   <i class="fa fa-home"></i> Back to Credit</button></center>
				   <?php
					}else{
				   ?>
				   			   <center>  <button style="margin-top:-40px" onclick="goTo( '<?php echo base_url('dashboard/credit_report_sales_rep'); ?>' )" class="btn btn-sm btn-default">
				   <i class="fa fa-home"></i> Back to Credit</button></center>
				   <?php
					}
				   ?>
		...
		<div class="login-form-area login-bg">
			<div class="row">
				...
				<div class="col-lg-2">
				  ...
				  <div class="col-lg-2">
				  <input type="hidden" name="user_id" id="method" value="1"/>
				</div>
			</div>
<br/>		
<div class="row">
		<form action="<?php echo base_url("dashboard/creditSales");  ?>" id="cart_form" method="POST">
...
<div class="btn-group btn-group-justified" role="group" aria-label="...">
			<div class="btn-group ng-scope" role="group" ng-controller="payBox">
			 <button type="button" class="btn btn-primary btn-lg sendToKitchenButton" onclick="PayNow()" style="margin-bottom:0px;"> <i class="fa fa-money"></i>
				  <span class="hidden-xs">Credit Sales</span>
			 </button>
			</div>
...
<?php  $extra_charges=$this->db->from("others")->where("SN","1")->get()->result_array()[0]; ?>
...
/*
if(to_pay > <?php echo $extra_charges['credit_limit'] ?>){
	error('Credit Sales Total is more than <?php echo number_format($extra_charges['credit_limit'],2) ?>, transaction will not be process when submitted');
}
*/
	...
	if(getSubTotal() ==0){
		...
		ajaxSentRequest($("#cart_form").attr('action'),data,function(result){
			...
			if(result.status==true){
				...
				refresh_stock_list("Credit Sales");				
			}else{
