	...
	<div class="col-sm-12">
		...
		<div class="panel panel-default">
					...
					<form method="post" class="form-horizontal" id="change_branch" action="">
						...
						?>
							...
							<div class="col-md-2">
								...
								<select name="customer_id2" class="form-control select2_demo_2 input-sm" id="customer_id">
                                ...
                                $stocks1 =$this->db->get()->result_array();
									foreach($stocks1 as $stock){
								?>
									<option <?php echo ($stock['SN']==$_POST['customer_id2'] ? 'selected' : '') ?> value="<?php echo $stock['SN'] ?>"><?php echo $stock['username'] ?></option>
								<?php
									}
								?>
								</select>
				...
				<tbody>
                    ...
                    $user_id = $this->tank_auth->get_user_id();
                    $user = $this->db->get_where("users",array("SN"=>$user_id))->row_array();
                    $filter =  array('payment_method'=>$customer_id, 'user_id'=>$customer_id2);
                    if($user['department'] !="Top Administrator"){
